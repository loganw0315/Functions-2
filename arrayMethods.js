...
// CODE HERE
const postTaxPrices = prices.map(num => ((num * 1.07).toFixed(2)))
...
// CODE HERE
const orderTotals = orders.map(order => (order.price + (order.price * order.tax)).toFixed(2));
...
// CODE HERE
const bobsTotal = purchases.map(purch => (purch.price)).reduce((a,c) => a + c);